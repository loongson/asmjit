...
#ifndef ASMJIT_CORE_LOONGARCHCOMMONS_H_INCLUDED
#define ASMJIT_CORE_LOONGARCHCOMMONS_H_INCLUDED

// This file provides architecture-specific classes that are required in the
// core library. For example Imm operand allows to be created from loong::Shift
// in a const-expr way, so the loong::Shift must be provided. So this header
...
//! \addtogroup asmjit_loong
//! \{

//! Represents LOONG immediate shift operation type and value.
class loongShift {
public:
  //! Operation predicate (LOONG) describes either SHIFT or EXTEND operation.
  //!
  ...
  inline loongShift() noexcept = default;

  //! Copy constructor (default)
  constexpr loongShift(const loongShift& other) noexcept = default;

  //! Constructs Shift from operation `op` and shift `value`.
  constexpr loongShift(uint32_t op, uint32_t value) noexcept
    : _op(op),
      _value(value) {}
...
static constexpr loongShift lsl(uint32_t value) noexcept { return loongShift(loongShift::kOpLSL, value); }
//! Constructs a `LSR #value` shift (logical shift right).
static constexpr loongShift lsr(uint32_t value) noexcept { return loongShift(loongShift::kOpLSR, value); }
//! Constructs a `ASR #value` shift (arithmetic shift right).
static constexpr loongShift asr(uint32_t value) noexcept { return loongShift(loongShift::kOpASR, value); }
//! Constructs a `ROR #value` shift (rotate right).
static constexpr loongShift ror(uint32_t value) noexcept { return loongShift(loongShift::kOpROR, value); }
//! Constructs a `RRX` shift (rotate with carry by 1).
static constexpr loongShift rrx() noexcept { return loongShift(loongShift::kOpRRX, 0); }
//! Constructs a `MSL #value` shift (logical shift left filling ones).
static constexpr loongShift msl(uint32_t value) noexcept { return loongShift(loongShift::kOpMSL, value); }

//! Constructs a `UXTB #value` extend and shift (unsigned byte extend).
static constexpr loongShift uxtb(uint32_t value) noexcept { return loongShift(loongShift::kOpUXTB, value); }
//! Constructs a `UXTH #value` extend and shift (unsigned hword extend).
static constexpr loongShift uxth(uint32_t value) noexcept { return loongShift(loongShift::kOpUXTH, value); }
//! Constructs a `UXTW #value` extend and shift (unsigned word extend).
static constexpr loongShift uxtw(uint32_t value) noexcept { return loongShift(loongShift::kOpUXTW, value); }
//! Constructs a `UXTX #value` extend and shift (unsigned dword extend).
static constexpr loongShift uxtx(uint32_t value) noexcept { return loongShift(loongShift::kOpUXTX, value); }

//! Constructs a `SXTB #value` extend and shift (signed byte extend).
static constexpr loongShift sxtb(uint32_t value) noexcept { return loongShift(loongShift::kOpSXTB, value); }
//! Constructs a `SXTH #value` extend and shift (signed hword extend).
static constexpr loongShift sxth(uint32_t value) noexcept { return loongShift(loongShift::kOpSXTH, value); }
//! Constructs a `SXTW #value` extend and shift (signed word extend).
static constexpr loongShift sxtw(uint32_t value) noexcept { return loongShift(loongShift::kOpSXTW, value); }
//! Constructs a `SXTX #value` extend and shift (signed dword extend).
static constexpr loongShift sxtx(uint32_t value) noexcept { return loongShift(loongShift::kOpSXTX, value); }

//! \}

ASMJIT_END_SUB_NAMESPACE

#endif // ASMJIT_CORE_LOONGARCHCOMMONS_H_INCLUDED
