...
UNIT(la64_operand) {
  INFO("Checking if la64::reg(...) matches built-in IDs");
  //EXPECT(w(5) == w5);
  EXPECT(x(5) == r5);
  ...
  EXPECT(r4.isReg() == true);
  //EXPECT(w0.id() == 0);
  EXPECT(r4.id() == 4);
  //EXPECT(wzr.id() == Gp::kIdZr);
  EXPECT(r0.id() == Gp::kIdZr);
  //EXPECT(wsp.id() == Gp::kIdSp);
  EXPECT(r3.id() == Gp::kIdSp);
  //EXPECT(w0.size() == 4);
  EXPECT(r4.size() == 8);
  //EXPECT(w0.type() == Reg::kTypeGpW);
  EXPECT(r4.type() == Reg::kTypeGpX);
  //EXPECT(w0.group() == Reg::kGroupGp);
  EXPECT(r4.group() == Reg::kGroupGp);
