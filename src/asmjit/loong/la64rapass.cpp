...
void ARMRAPass::onInit() noexcept {
  ...
  _scratchRegIndexes[0] = uint8_t(30);
  _scratchRegIndexes[1] = uint8_t(31);
  ...
  makeUnavailable(Reg::kGroupGp, Gp::kIdTp); // OS-specific use, usually TLS.
...
Error ARMRAPass::updateStackFrame() noexcept {
  if (_func->frame().hasFuncCalls())
    ...
    _func->frame().addDirtyRegs(Reg::kGroupGp, Support::bitMask(Gp::kIdRa));
