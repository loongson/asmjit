...
#ifndef ASMJIT_LOONG_LOONGOPERAND_H_INCLUDED
#define ASMJIT_LOONG_LOONGOPERAND_H_INCLUDED
...
ASMJIT_BEGIN_SUB_NAMESPACE(loong)
...
ASMJIT_DEFINE_REG_TRAITS(VecX , BaseReg::kTypeVec256 , BaseReg::kGroupVec  , 32, 32, Type::kIdI32x8 );
//! \endcond
...
public:
  ...
  enum RegType : uint32_t {
    ...
    //! 256-bit view of LASX register (X)
    kTypeVecX = BaseReg::kTypeVec256,
  ...
  //! Gets whether the register is a VEC-X register (256-bit).
  constexpr bool isVecX() const noexcept { return hasBaseSignature(RegTraits<kTypeVecX>::kSignature); }

  //! Gets whether the register is either VEC-D (64-bit) or VEC-Q (128-bit) or VEC-X (256-bit).
  ...
  static inline uint32_t groupOf(uint32_t rType) noexcept { return _archTraits[Environment::kArchLOONGARCH64].regTypeToGroup(rType); }
  static inline uint32_t typeIdOf(uint32_t rType) noexcept { return _archTraits[Environment::kArchLOONGARCH64].regTypeToTypeId(rType); }
  static inline uint32_t signatureOf(uint32_t rType) noexcept { return _archTraits[Environment::kArchLOONGARCH64].regTypeToSignature(rType); }
  ...
  static inline bool isVecX(const Operand_& op) noexcept { return op.as<Reg>().isVecX(); }
  ...
  static inline bool isVecX(const Operand_& op, uint32_t id) noexcept { return isVecX(op) & (op.id() == id); }
};
...
public:
  ...
  enum Id : uint32_t {
    ...
    //! Frame pointer.
    kIdFp = 22,
    //! Link register. loongarch is ra register
    ////kIdLr = 1,
    kIdRa = 1,
    //! Stack register id.
    kIdSp = 3,
...
public:
  ...
  constexpr bool isVecB32() const noexcept { return _hasSignaturePart<kBaseSignature | kSignatureRegElementTypeMask>(RegTraits<kTypeVecX>::kSignature | kSignatureElementB); }
  constexpr bool isVecH16() const noexcept { return _hasSignaturePart<kBaseSignature | kSignatureRegElementTypeMask>(RegTraits<kTypeVecX>::kSignature | kSignatureElementH); }
  constexpr bool isVecS8() const noexcept { return _hasSignaturePart<kBaseSignature | kSignatureRegElementTypeMask>(RegTraits<kTypeVecX>::kSignature | kSignatureElementS); }
  constexpr bool isVecD4() const noexcept { return _hasSignaturePart<kBaseSignature | kSignatureRegElementTypeMask>(RegTraits<kTypeVecX>::kSignature | kSignatureElementD); }
  ...
  static constexpr uint32_t _nmakeElementAccessSignature(uint32_t elementType, uint32_t elementIndex) noexcept {
    return uint32_t(RegTraits<kTypeVecX>::kSignature)               |
           uint32_t(kSignatureRegElementFlagMask)                   |
           uint32_t(elementType << kSignatureRegElementTypeShift)   |
           uint32_t(elementIndex << kSignatureRegElementIndexShift) ;
  }
...
{
  ASMJIT_DEFINE_FINAL_REG(VecV, Vec, RegTraits<kTypeVecV>)
  inline VecD half() const noexcept { return VecD(id()); }
};
//! 256-bit vector register (X).
class VecX : public Vec
{
  ASMJIT_DEFINE_FINAL_REG(VecX, Vec, RegTraits<kTypeVecX>)
  inline VecV half() const noexcept { return VecV(id()); }
};
...
inline VecX Vec::lx() const noexcept { return VecX(id()); }
...
inline VecX Vec::bx(uint32_t elementIndex) const noexcept { return VecX::fromSignatureAndId(_nmakeElementAccessSignature(kElementTypeB, elementIndex), id()); }
...
inline VecX Vec::hx(uint32_t elementIndex) const noexcept { return VecX::fromSignatureAndId(_nmakeElementAccessSignature(kElementTypeH, elementIndex), id()); }
...
inline VecX Vec::sx(uint32_t elementIndex) const noexcept { return VecX::fromSignatureAndId(_nmakeElementAccessSignature(kElementTypeS, elementIndex), id()); }
...
inline VecX Vec::dx(uint32_t elementIndex) const noexcept { return VecX::fromSignatureAndId(_nmakeElementAccessSignature(kElementTypeD, elementIndex), id()); }
...
inline VecX Vec::h2x(uint32_t elementIndex) const noexcept { return VecX::fromSignatureAndId(_nmakeElementAccessSignature(kElementTypeH2, elementIndex), id()); }
...
inline VecX Vec::b4x(uint32_t elementIndex) const noexcept { return VecX::fromSignatureAndId(_nmakeElementAccessSignature(kElementTypeB4, elementIndex), id()); }
...
inline VecX Vec::b32() const noexcept { return VecX::fromSignatureAndId(VecX::kSignature | kSignatureElementB, id()); }
inline VecX Vec::h16() const noexcept { return VecX::fromSignatureAndId(VecX::kSignature | kSignatureElementH, id()); }
inline VecX Vec::s8() const noexcept { return VecX::fromSignatureAndId(VecX::kSignature | kSignatureElementS, id()); }
inline VecX Vec::d4() const noexcept { return VecX::fromSignatureAndId(VecX::kSignature | kSignatureElementD, id()); }
...
//! Creates a 128-bit V register operand (ARM/AArch64/LOONGARCH).
static constexpr VecV v(uint32_t id) noexcept { return VecV(id); }
//! Creates a 256-bit V register operand (ARM/AArch64/LOONGARCH).
static constexpr VecX lx(uint32_t id) noexcept { return VecX(id); }
...
public:
  ...
  constexpr Mem(const BaseReg& base, const BaseReg& index, const loongShift& shift, uint32_t flags = 0) noexcept
...
static constexpr Mem ptr_post(const Gp& base, const Gp& index) noexcept {
  return Mem(base, index, loongShift(loongShift::kOpLSL, 0), Mem::kOffsetPostIndex << Mem::kSignatureMemPredicateShift);
}

//! Creates `[base.reg, index, SHIFT_OP #shift]` memory operand.
static constexpr Mem ptr(const Gp& base, const Gp& index, const loongShift& shift) noexcept {
...
ASMJIT_DEFINE_TYPE_ID(loong::GpW, kIdI32);
ASMJIT_DEFINE_TYPE_ID(loong::GpX, kIdI64);
ASMJIT_DEFINE_TYPE_ID(loong::VecS, kIdF32x1);
ASMJIT_DEFINE_TYPE_ID(loong::VecD, kIdF64x1);
ASMJIT_DEFINE_TYPE_ID(loong::VecV, kIdI32x4);
ASMJIT_DEFINE_TYPE_ID(loong::VecX, kIdI32x8);
ASMJIT_END_NAMESPACE
//! \endcond

#endif // ASMJIT_LOONG_LOONGOPERAND_H_INCLUDED
