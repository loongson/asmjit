...
int main(int argc, char* argv[]) {
  ...
  if ((strcmp(arch, "all") == 0 || strcmp(arch, "loongarch64") == 0))
    la64Failed = !testLA64Assembler(settings);
#endif

  bool failed = x86Failed || x64Failed || la64Failed;

  if (failed) {
    if (x86Failed) printf("** X86 test suite failed **\n");
    if (x64Failed) printf("** X64 test suite failed **\n");
    if (la64Failed) printf("** Loongarch64 test suite failed **\n");
    printf("** FAILURE **\n");
  }
  else {
    printf("** SUCCESS **\n");
