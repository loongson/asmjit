...
uint64_t flagBit = 0;
const uint64_t XREG = 1ULL << flagBit++; /** 通用寄存器 */
...
class Test {
  ...
  std::vector<std::string> tv_XREG = {"$zero", "$ra", "$tp", "$a0", "$a4", "$t4", "$s8"};
  std::vector<std::string> jtv_XREG = {"$r0", "$r1", "$r2", "$r4", "$r8", "$r16", "$r31"};
  std::vector<std::string> tv_VREG = {"$vr11", "$vr0", "$vr4", "$vr8", "$vr16", "$vr31"};
  std::vector<std::string> tv_XVREG = {"$xr0", "$xr1", "$xr2", "$xr4", "$xr8", "$xr16", "$xr31"};
  std::vector<std::string> jtv_VREG = {"v11", "v0", "v4", "v8", "v16", "v31"};
  ...
  std::vector<std::vector<std::string> *> tv_VectorsAs = {&tv_XREG,
                                                          ...
                                                          &tv_SPECIFIC32_3,
                                                          &tv_SPECIFIC64_3};
  std::vector<std::vector<std::string> *> tv_VectorsJit = {&jtv_XREG,
                                                           &jtv_VREG,
                                                           &tv_XVREG,
                                                           &tv_MSACREG,
                                                           &tv_IMM1BIT,
  ...
  */
  ...
  void put(const char *nm, std::vector<uint64_t> &ops) const {
    ...
    if (num_ops == 0) {
      if (isAsmjit_) {
        std::cout << "a.";  //a.nop(), la64::Assembler a(&code);
      }
